...
if [ $SKIP_BUILD -eq 0 ]; then
    pushd_cmd $SCRIPT_DIR/..
    run_cmd "mvn clean package"
    popd_cmd
    mkdir -p $DIST_DIR
    rm -rf $DIST_DIR/*

    run_cmd "cp -p $SCRIPT_DIR/../nifi-GPUdbNiFi-nar/target/*.nar $DIST_DIR"
    run_cmd "cp -p $SCRIPT_DIR/../README.md $DIST_DIR"
    run_cmd "sed -i s@\"gpudb-connector-nifi/nifi-GPUdbNiFi-nar/target/\"@@g $DIST_DIR/README.md"
fi
...
# Grab version
pushd $DIST_DIR
VERSION=$(find *.nar | awk -F'-' '{print $4}' )
popd
...
SPEC_FILE=$RPM_BUILD_DIR/SPECS/gpudb-connector-nifi.spec
run_cmd "cp $SCRIPT_DIR/gpudb-connector-nifi.spec $SPEC_FILE"
