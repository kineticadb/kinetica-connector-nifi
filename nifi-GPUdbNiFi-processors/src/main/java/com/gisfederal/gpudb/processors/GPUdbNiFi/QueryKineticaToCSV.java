...
@Tags({"Kinetica", "query", "get", "csv"})
@CapabilityDescription("Queries a Kinetica table or executes a SQL query and outputs the results as CSV. "
...
public class QueryKineticaToCSV extends AbstractProcessor {
    ...
    public static final PropertyDescriptor PROP_DELIMITER = new PropertyDescriptor.Builder()
            ...
            .description("Delimiter character for CSV output (defaults to ',')")
            .required(true)
            .defaultValue(",")
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
    ...
    public static final Relationship REL_SUCCESS = new Relationship.Builder()
            ...
            .description("FlowFiles with query results in CSV format")
            .build();
    ...
    protected void init(final ProcessorInitializationContext context) {
        ...
        descriptorsList.add(PROP_DELIMITER);
        descriptorsList.add(PROP_USERNAME);
        descriptorsList.add(PROP_PASSWORD);
        descriptorsList.add(PROP_DISABLE_AUTO_DISCOVERY);
        descriptorsList.add(PROP_DISABLE_FAILOVER);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            while (hasMore) {
                ...
                final char currentDelimiter = delimiter;

                FlowFile flowFile = session.create();
                flowFile = session.write(flowFile, (OutputStream out) -> {
                    writeCSV(out, records, currentType, currentDelimiter);
                });
                ...
                getLogger().info("Batch {} - wrote {} CSV records from {} at {}",
    ...
    private void writeCSV(OutputStream out, List<Record> records, Type type, char delim) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(out));
             CSVPrinter printer = new CSVPrinter(writer, CSVFormat.RFC4180.builder().setDelimiter(delim).build())) {

            List<Type.Column> columns = type.getColumns();

            // Header row
            List<String> headers = new ArrayList<>();
            for (Type.Column column : columns) {
