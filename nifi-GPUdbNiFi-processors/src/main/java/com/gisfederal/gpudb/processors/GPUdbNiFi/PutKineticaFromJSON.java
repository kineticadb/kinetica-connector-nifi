...
@Tags({"Kinetica", "add", "bulkadd", "put", "json"})
@CapabilityDescription("Reads JSON content from a FlowFile and inserts records into a Kinetica table via the native "
        + "GPUdb BulkInserter API. The FlowFile content may be a JSON array of objects or a single JSON object. "
        + "Each JSON object's keys must correspond to the column names of the target Kinetica table. "
...
public class PutKineticaFromJSON extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try (InputStream istream = session.read(flowFile)) {
            ...
            JsonNode rootNode = mapper.readTree(istream);

            if (rootNode == null || rootNode.isMissingNode()) {
                getLogger().warn(PROCESSOR_NAME + " Warning: Empty JSON content in FlowFile.");
            ...
            if (rootNode.isArray()) {
                for (JsonNode node : rootNode) {
                    jsonObjects.add(node);
                }
            } else if (rootNode.isObject()) {
                jsonObjects.add(rootNode);
            } else {
                throw new ProcessException(PROCESSOR_NAME + " Error: FlowFile content is neither a JSON array nor a JSON object.");
            }
            ...
            for (JsonNode jsonObj : jsonObjects) {
                ...
                for (int i = 0; i < numColumns; i++) {
                    ...
                    JsonNode valueNode = jsonObj.get(colName);
                    ...
                    try {
                        if (valueNode == null || valueNode.isNull()) {
                            ...
                            if (timeStamp) {
                                ...
                                object.put(colName, valueNode.isNumber() ? valueNode.doubleValue()
                                        : Double.parseDouble(valueStr));
                            } else if (column.getType() == Float.class) {
                                object.put(colName, valueNode.isNumber() ? valueNode.floatValue()
                                        : Float.parseFloat(valueStr));
                            } else if (column.getType() == Integer.class) {
                                object.put(colName, valueNode.isNumber() ? valueNode.intValue()
                                        : Integer.parseInt(valueStr));
                            } else if (column.getType() == Long.class) {
                                object.put(colName, valueNode.isNumber() ? valueNode.longValue()
                                        : Long.parseLong(valueStr));
                            } else {
                                String strVal = valueNode.isTextual() ? valueNode.asText() : valueNode.toString();
                                if (!strVal.trim().isEmpty()) {
                                    object.put(colName, strVal.trim());
                                }
                            }
                        }
        ...
        if (failed) {
            flowFile = session.putAttribute(flowFile, "error.message",
                    PROCESSOR_NAME + ": Failed to process JSON content");
